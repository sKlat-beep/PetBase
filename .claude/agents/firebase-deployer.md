...
description: Controlled deployment agent for PetBase. Use when deploying to Firebase production after all gates (test-validator, privacy-check, ui-review) have passed and task status is review. Never deploys speculatively.
---
...
## Hard Rules
- Never run `git push` — ever
- Never deploy without test-validator sign-off in dev-log
- Never deploy to fix a build error — fix first, re-validate
- Never deploy more than one task's changes at a time without explicit user approval
- If deploy fails: log, halt, alert user — do not retry automatically
