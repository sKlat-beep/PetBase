  ...
  return (
    <section className="bg-white/80 dark:bg-stone-800/80 backdrop-blur-sm rounded-2xl border border-stone-100 dark:border-stone-700 p-5">
        ...
        {showSearch && (
          ...
          >
            <input
              ...
              className="w-full rounded-xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-800 px-4 py-2.5 text-sm text-stone-800 dark:text-stone-200 placeholder-stone-400 dark:placeholder-stone-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 min-h-[44px]"
              autoFocus
            />
          </motion.div>
        )}
      ...
      {pymkSuggestions.length > 0 && !query.trim() && (
        <div className="mb-4">
          <h3 className="flex items-center gap-1.5 text-xs font-semibold text-stone-500 dark:text-stone-400 uppercase tracking-wide mb-2">
          ...
          <div className="overflow-x-auto pb-1 -mx-1 px-1">
            <div className="flex gap-3" style={{ minWidth: 'max-content' }}>
              {pymkSuggestions.map(suggestion => (
                <div
                  key={suggestion.uid}
                  className="flex flex-col items-center gap-1.5 bg-stone-50 dark:bg-stone-700/50 border border-stone-100 dark:border-stone-700 rounded-xl p-3 w-28 shrink-0"
                >
                  <img
                    src={suggestion.avatarUrl || `https://api.dicebear.com/7.x/thumbs/svg?seed=${suggestion.uid}`}
                    alt={suggestion.displayName}
                    className="w-12 h-12 rounded-full object-cover bg-stone-200 dark:bg-stone-600"
                    referrerPolicy="no-referrer"
                  />
                  <p className="text-xs font-semibold text-stone-900 dark:text-stone-100 text-center truncate w-full leading-tight">
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-1 py-1 px-2 rounded-lg bg-stone-900 dark:bg-stone-100 text-white dark:text-stone-900 hover:bg-stone-700 dark:hover:bg-stone-200 font-medium text-[11px] transition-colors"
                  >
