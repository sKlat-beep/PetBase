  ...
  return (
    ...
    >
      ...
      <div className="space-y-4">
          ...
          <div className="flex gap-2 flex-1">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder={`Search ${activeTab.toLowerCase()} by name, service, or keyword...`}
                className="w-full pl-12 pr-4 py-3 rounded-xl border border-stone-200 dark:border-stone-600 bg-white dark:bg-stone-800 text-stone-900 dark:text-stone-100 placeholder:text-stone-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-shadow"
              />
            </div>
            ...
            <div className="relative w-32 md:w-48">
              <button
                ...
                className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-400 hover:text-emerald-500 transition-colors bg-transparent border-none p-1 cursor-pointer"
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-3 rounded-xl border border-stone-200 dark:border-stone-600 bg-white dark:bg-stone-800 text-stone-900 dark:text-stone-100 placeholder:text-stone-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-shadow"
              />
            </div>
          </div>
          ...
          <div className="flex gap-2">
            {location && profile && location !== profile.zipCode && (
              <button
                ...
                className="px-4 py-3 text-sm font-medium bg-stone-900 dark:bg-stone-100 text-white dark:text-stone-900 rounded-xl hover:bg-stone-800 dark:hover:bg-stone-200 transition-colors whitespace-nowrap"
              >
                Make Permanent
              </button>
            )}
        ...
        {/* Filter Drawer */}
        {showFilters && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="bg-white dark:bg-stone-800 rounded-xl p-4 border border-stone-100 dark:border-stone-700 shadow-sm overflow-hidden">
            <div className="space-y-6">
              <div>
                <h4 className="text-sm font-semibold text-stone-900 dark:text-stone-100 mb-2">My Pets (Smart Filter)</h4>
                {availablePetTypes.length === 0 ? (
                  <p className="text-sm text-stone-400 dark:text-stone-500">Add pets to your profile to enable smart filtering.</p>
                ) : (
              ...
              {activeTab !== 'Stores' && (
                <div>
                  <h4 className="text-sm font-semibold text-stone-900 dark:text-stone-100 mb-2">Service Capabilities</h4>
                  <div className="flex flex-wrap gap-2">
          ...
          <button
            ...
            className={`
              ...
              ${activeTab === tab
                ...
                : 'bg-white dark:bg-stone-800 border border-stone-200 dark:border-stone-600 text-stone-600 dark:text-stone-400 hover:bg-stone-50 dark:hover:bg-stone-700'
              }
            `}
          >
            {tab}
      ...
      {loading ? (
        ...
        <div className="space-y-8 tracking-tight">
          {recentInteractions.length > 0 && (
            <div>
              <h3 className="text-xl font-bold text-stone-900 dark:text-stone-100 flex items-center gap-2 mb-4">
                <Clock className="w-5 h-5 text-emerald-600" /> Recently Viewed
              </h3>
              <div className="flex gap-2 flex-wrap">
                {recentInteractions.map(interaction => (
                  <span key={`${interaction.type}-${interaction.id}`} className="px-3 py-1.5 bg-stone-100 dark:bg-stone-800 border border-stone-200 dark:border-stone-700 rounded-full text-sm text-stone-700 dark:text-stone-300 flex items-center gap-1.5 cursor-pointer hover:bg-stone-200 dark:hover:bg-stone-700 transition">
          ...
          <div>
            <h3 className="text-xl font-bold text-stone-900 dark:text-stone-100 mb-4">Popular Local Websites</h3>
            <div className="flex overflow-x-auto pb-4 -mx-4 px-4 md:mx-0 md:px-0 gap-4 snap-x">
              {websiteResults.sort((a, b) => favoriteWebsites.includes(b.id) ? 1 : favoriteWebsites.includes(a.id) ? -1 : 0).map(site => (
                <div key={site.id} onClick={() => recordInteraction(site.id, 'website', site.name)} className="snap-start shrink-0 w-64 bg-white dark:bg-stone-800 rounded-2xl p-5 border border-stone-100 dark:border-stone-700 shadow-sm hover:shadow-md transition cursor-pointer flex flex-col items-start gap-4">
                  <div className="w-full flex justify-between items-start">
                    <div className="w-12 h-12 bg-stone-50 dark:bg-stone-700 rounded-xl flex items-center justify-center p-2 border border-stone-200 dark:border-stone-600">
                      <img src={site.logo} alt={site.name} className="max-w-full max-h-full object-contain mix-blend-multiply dark:mix-blend-normal" />
                    </div>
                    <button onClick={(e) => { e.stopPropagation(); toggleFavorite(site.id, 'website'); }} className="p-1.5 hover:bg-stone-100 dark:hover:bg-stone-700 rounded-full transition text-stone-400">
                      <Heart className={`w-5 h-5 ${favoriteWebsites.includes(site.id) ? 'fill-rose-500 text-rose-500' : ''}`} />
                    </button>
                  </div>
                  <div>
                    <h4 className="font-bold text-stone-900 dark:text-stone-100">{site.name}</h4>
                    <p className="text-xs text-stone-500 dark:text-stone-400 mt-1 line-clamp-2">{site.description}</p>
                  </div>
          ...
          <div>
            <h3 className="text-xl font-bold text-stone-900 dark:text-stone-100 mb-4">Nearby Stores & Supplies</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {storeResults.sort((a, b) => favoriteStores.includes(b.id) ? 1 : favoriteStores.includes(a.id) ? -1 : 0).map(store => (
                <div key={store.id} onClick={() => recordInteraction(store.id, 'store', store.name)} className="bg-white dark:bg-stone-800 rounded-2xl overflow-hidden cursor-pointer shadow-sm border border-stone-100 dark:border-stone-700 hover:shadow-md transition">
                  <div className="h-40 relative">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); toggleFavorite(store.id, 'store'); }} className="absolute top-3 right-3 p-2 bg-white/50 backdrop-blur-md hover:bg-white/80 rounded-full transition shadow-sm">
                      <Heart className={`w-5 h-5 ${favoriteStores.includes(store.id) ? 'fill-rose-500 text-rose-500' : 'text-stone-700'}`} />
                    </button>
                  ...
                  <div className="p-4">
                    ...
                    <h4 className="font-bold text-lg text-stone-900 dark:text-stone-100">{store.name}</h4>
                    <div className="flex items-center gap-1.5 text-sm text-stone-500 dark:text-stone-400 mt-2">
                      <MapPin className="w-4 h-4" /> {store.address} ({store.distance})
                    </div>
                    <div className="flex flex-wrap gap-1.5 mt-3">
                      {store.tags.map(tag => (
                        <span key={tag} className="px-2 py-1 bg-stone-100 dark:bg-stone-700/50 text-stone-600 dark:text-stone-400 rounded-md text-[11px] font-medium">
                          {tag}
                        </span>
                      ))}
                    </div>
        ...
        <div className="py-20 text-center bg-white dark:bg-stone-800 rounded-2xl border-2 border-dashed border-stone-200 dark:border-stone-700">
          <SearchIcon className="w-8 h-8 mx-auto mb-3 text-stone-300 dark:text-stone-600" />
          <h3 className="text-lg font-medium text-stone-900 dark:text-stone-100">No results found</h3>
          <p className="text-stone-500 dark:text-stone-400">Try adjusting your filters or searching a different area.</p>
        </div>
      ) : (
        <div>
          {/* Sponsored section */}
          {displayedResults.filter(r => r.isSponsored).length > 0 && (
            <div className="mb-4">
              <p className="text-xs font-semibold text-stone-400 dark:text-stone-500 uppercase tracking-wide mb-2 px-1">Sponsored</p>
              <div className="flex gap-3 overflow-x-auto pb-2 -mx-1 px-1">
                {displayedResults.filter(r => r.isSponsored).map(r => (
                  <button key={r.id} onClick={() => setSelectedService(r)}
                    ...
                    <p className="text-xs font-semibold text-stone-800 dark:text-stone-200 truncate">{r.name}</p>
                    <p className="text-[10px] text-stone-500 dark:text-stone-400 truncate">{r.address}</p>
                  </button>
                ))}
              </div>
            </div>
          ...
          {!loading && displayedResults.length === 0 && activeTab !== 'Saved' && (
            <div className="py-16 flex flex-col items-center gap-3 text-stone-500 dark:text-stone-400">
              <MapPin className="w-10 h-10 text-stone-300 dark:text-stone-600" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {displayedResults.map((result) => (
              <div
                ...
                className="bg-white dark:bg-stone-800 rounded-2xl overflow-hidden shadow-sm border border-stone-100 dark:border-stone-700 hover:shadow-md transition-shadow flex flex-col cursor-pointer"
              >
                <div className="h-48 relative">
                  ...
                  <button
                    ...
                    className={`absolute top-3 left-3 p-1.5 rounded-full backdrop-blur-sm transition-colors ${savedServiceIds.includes(result.id) ? 'bg-rose-500/90 text-white' : 'bg-white/70 text-stone-500 hover:bg-rose-50 hover:text-rose-500'}`}
                  >
                ...
                <div className="p-5 flex flex-col flex-1">
                  ...
                  <div className="flex items-center gap-1.5 text-sm text-stone-600 dark:text-stone-400 mb-4">
                    <MapPin className="w-4 h-4 text-stone-400 dark:text-stone-500 shrink-0" />
                    <span className="truncate">{result.address}</span>
                    <span className="text-stone-400 dark:text-stone-500 font-medium shrink-0">
                      ({result.distance})
                    </span>
                  </div>

                  <div className="flex flex-wrap gap-2 mt-auto pt-4 border-t border-stone-100 dark:border-stone-700">
                    {result.tags.map((tag) => (
                      <span
                        key={tag}
                        className="bg-stone-100 dark:bg-stone-700 text-stone-600 dark:text-stone-300 px-2.5 py-1 rounded-md text-xs font-medium"
                      >
                        {tag}
                      </span>
                    ))}
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="mt-4 pt-4 border-t border-stone-100 dark:border-stone-700" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center gap-1.5 mb-2">
                          <MessageSquare className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
                          <span className="text-xs font-bold text-stone-900 dark:text-stone-100 uppercase tracking-wider">Community Tips</span>
                        </div>
                        {allTips.length > 0 && (
                          <div className="space-y-2 mb-3">
                            {allTips.map((tip, idx) => (
                              <div key={idx} className="bg-stone-50 dark:bg-stone-700/30 rounded-lg p-3">
                                {tip.rating && tip.rating > 0 && (
                                  <div className="flex gap-0.5 mb-1">
                                    {[1,2,3,4,5].map(s => (
                                      <span key={s} className={`text-xs ${s <= tip.rating! ? 'text-amber-400' : 'text-stone-300 dark:text-stone-600'}`}>★</span>
                                    ))}
                                  </div>
                                )}
                                <p className="text-sm text-stone-700 dark:text-stone-300 italic">"{tip.text}"</p>
                                <div className="flex justify-between items-center mt-1.5">
                                  <span className="text-xs font-medium text-stone-600 dark:text-stone-400">— {tip.author}</span>
                                  <div className="flex items-center gap-2">
                                    <button
                                      onClick={(e) => { e.stopPropagation(); upvoteTip(result.id, idx); }}
                                      disabled={(tip.upvoters ?? []).includes(user?.uid ?? '')}
                                      className="flex items-center gap-0.5 text-[10px] text-stone-400 hover:text-emerald-600 disabled:opacity-40 disabled:cursor-default transition-colors"
                                      aria-label="Upvote tip"
                                    >
                                      ▲ {tip.upvotes ?? 0}
                                    </button>
                                    <span className="text-[10px] text-stone-400">{new Date(tip.date).toLocaleDateString()}</span>
                                  </div>
                                </div>
                              </div>
                            ))}
                        ...
                        >
                          <div className="flex gap-1 mb-1">
                            {[1,2,3,4,5].map(s => (
                              <button key={s} type="button" onClick={(ev) => { ev.stopPropagation(); setTipRating(prev => ({ ...prev, [result.id]: prev[result.id] === s ? 0 : s })); }}
                                className={`text-sm ${s <= (tipRating[result.id] ?? 0) ? 'text-amber-400' : 'text-stone-300 dark:text-stone-600'} hover:text-amber-400 transition-colors`}
                              >★</button>
                            ))}
                            <span className="text-[10px] text-stone-400 ml-1 self-center">
                          ...
                          <input
                            ...
                            className="flex-1 px-3 py-1.5 text-sm rounded-lg border border-stone-200 dark:border-stone-600 bg-white dark:bg-stone-800 text-stone-900 dark:text-stone-100 placeholder:text-stone-400 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                          />
                          <button
                            type="submit"
