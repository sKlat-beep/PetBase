    ...
    <div className="bg-amber-50/50 dark:bg-amber-950/20 rounded-xl border border-amber-200 dark:border-amber-800 p-4">
      ...
      {sorted.length === 0 ? (
        <div className="text-center py-3">
          <p className="text-xs text-neutral-500 dark:text-neutral-400 mb-2">No nominees this week</p>
        ...
        <div className="space-y-2">
          {sorted.slice(0, 5).map((n, idx) => (
            <div key={n.petId} className="flex items-center gap-3">
              ...
              {idx > 2 && <span className="w-6 text-center text-xs text-neutral-400">{idx + 1}</span>}
              {n.petImage ? (
                <img src={n.petImage} alt={n.petName} className="w-8 h-8 rounded-full object-cover" referrerPolicy="no-referrer" />
              ) : (
                <div className="w-8 h-8 rounded-full bg-neutral-200 dark:bg-neutral-600 flex items-center justify-center text-xs font-bold">
                  {n.petName.charAt(0)}
                </div>
              )}
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-neutral-800 dark:text-neutral-200 truncate">{n.petName}</p>
                <p className="text-[10px] text-neutral-400">by {n.ownerName}</p>
              </div>
              <button
      ...
      {nominating && (
        <div className="mt-3 border-t border-amber-200 dark:border-amber-800 pt-3 space-y-1">
          <p className="text-xs text-neutral-500 dark:text-neutral-400 mb-1">Choose a pet to nominate:</p>
          {pets.filter(p => !p.isPrivate).map(pet => (
            ...
            >
              {pet.image ? (
                ...
                <div className="w-6 h-6 rounded-full bg-neutral-200 dark:bg-neutral-600 text-[10px] font-bold flex items-center justify-center">
                  {pet.name.charAt(0)}
                </div>
              )}
              <span className="text-neutral-800 dark:text-neutral-200">{pet.name}</span>
            </button>
          ))}
        </div>
      )}
