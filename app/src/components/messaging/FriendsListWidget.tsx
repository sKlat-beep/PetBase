  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1.5">
        <p className="text-[10px] font-semibold text-neutral-400 dark:text-neutral-500 uppercase tracking-wide">Friends</p>
        <div className="flex gap-1">
          {(['recent', 'az'] as const).map(s => (
            <button key={s} onClick={() => handleSort(s)}
              className={`text-[10px] px-1.5 py-0.5 rounded transition-colors ${sort === s ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400' : 'text-neutral-400 dark:text-neutral-500 hover:text-neutral-600 dark:hover:text-neutral-300'}`}>
              {s === 'recent' ? 'Recent' : 'A–Z'}
            </button>
          ))}
        </div>
      ...
      <div className="space-y-0.5 max-h-48 overflow-y-auto">
        {sorted.slice(0, 20).map(uid => {
          ...
          return (
            <div key={uid} className="flex items-center gap-2 px-1.5 py-1.5 rounded-xl hover:bg-neutral-50 dark:hover:bg-neutral-700/50 group transition-colors relative">
              ...
              <div className="flex-1 min-w-0 flex items-center gap-1">
                <span className="text-xs font-medium text-neutral-700 dark:text-neutral-200 truncate">{p.displayName}</span>
                {isMuted && <VolumeX className="w-3 h-3 text-neutral-400 shrink-0" />}
              </div>
              <div className="hidden group-hover:flex gap-1 shrink-0">
                <button onClick={() => setActiveUid(uid)}
                  className="p-1 rounded-lg text-neutral-400 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-neutral-100 dark:hover:bg-neutral-700 transition-colors">
                  <MessageCircle className="w-3.5 h-3.5" />
                </button>
                <button onClick={() => setMenuUid(menuUid === uid ? null : uid)}
                  className="p-1 rounded-lg text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-300 hover:bg-neutral-100 dark:hover:bg-neutral-700 transition-colors">
                  <MoreHorizontal className="w-3.5 h-3.5" />
                </button>
              </div>
              {menuUid === uid && (
                <div className="absolute right-0 top-full mt-1 z-20 bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl shadow-xl py-1 min-w-[120px]">
                  <button onClick={() => handleMute(uid)}
                    className="w-full px-3 py-2 text-left text-xs text-neutral-700 dark:text-neutral-200 hover:bg-neutral-50 dark:hover:bg-neutral-700 transition-colors flex items-center gap-2">
